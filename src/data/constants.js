import amoeba from "../images/amoeba.jpg"
import railways from "../images/railways.jpeg"
import camera_360 from "../images/360camera.jpeg"
export const Bio = {
  name: "Biswajit Sahu",
  roles: [
    "Android Developer",
    "Web Developer",
    "Electronics Technician",
    "Programmer",
  ],
  description:
    "I embody a strong drive and adaptability, consistently seeking fresh opportunities to conquer. Fueled by an insatiable thirst for knowledge, I remain unwavering in my commitment to produce outcomes of the utmost excellence. Guided by optimism and a mindset primed for advancement, I stand prepared to effect substantial change and attain remarkable accomplishments. - chat GPT",
  github: "https://github.com/Biswa1045",
  resume:
    "https://drive.google.com/file/d/1uHvd-e-XBw9f7vpbaZYgBVx4sllsbo9f/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/biswajit-1045s/",
  insta: "https://www.instagram.com/iam_biswajiit/",
  
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
     
      {
        name: "Next Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      {
        name: "Material UI",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Flutter",
        image:
          "https://cdn-images-1.medium.com/max/1200/1*5-aoK8IBmXve5whBQM90GA.png",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "data:image/png;base64,",
      },
     
      {
        name: "Python",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg",
      },
      {
        name: "MySQL",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      },
      
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      {
        name: "Firebase",
        image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      },
    ],
  },
  {
    title: "Android",
    skills: [
      {
        name: "Java",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg",
      },
      {
        name: "Kotlin",
        image:
          "https://www.vectorlogo.zone/logos/kotlinlang/kotlinlang-icon.svg",
      },
      {
        name: "XML",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSBMw6_RdwKQ9bDFfnKDX1iwMl4bVJEvd9PP53XuIw&s",
      },
      {
        name: "Android Studio",
        image:
          "https://developer.android.com/static/studio/images/new-studio-logo-1_1920.png",
      },
    ],
  },
  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image:
          "https://camo.githubusercontent.com/fbfcb9e3dc648adc93bef37c718db16c52f617ad055a26de6dc3c21865c3321d/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f6769742d73636d2f6769742d73636d2d69636f6e2e737667",
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
    
      
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      {
        name: "Postman",
        image:
          "https://camo.githubusercontent.com/93b32389bf746009ca2370de7fe06c3b5146f4c99d99df65994f9ced0ba41685/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f676574706f73746d616e2f676574706f73746d616e2d69636f6e2e737667",
      },
      {
        name: "Adobe XD",
        image:
          "https://camo.githubusercontent.com/c205ecbe12500177d102169d97bc1c17c545155fdf5ec78c08d54ac53e5b38c1/68747470733a2f2f63646e2e776f726c64766563746f726c6f676f2e636f6d2f6c6f676f732f61646f62652d78642e737667",
      },
      {
        name: "Figma",
        image:
          "https://camo.githubusercontent.com/ed93c2b000a76ceaad1503e7eb9356591b885227e82a36a005b9d3498b303ba5/68747470733a2f2f7777772e766563746f726c6f676f2e7a6f6e652f6c6f676f732f6669676d612f6669676d612d69636f6e2e737667",
      },
    ],
  },
];

export const experiences = [
  
  {
    id: 0,
    img: "https://media.licdn.com/dms/image/C560BAQF3CMO3ixFPFQ/company-logo_100_100/0/1677860604982?e=1700092800&v=beta&t=G6VJlqct9mrN0zhuykkGF1DQiSAfXZGTlga-3GKXBdU",
    role: "Student Ambassador",
    company: "IIEC - IGIT",
    date: "2023 - Present",
    desc: "Student Ambassador/Co-ordinator of Idea Innovation and Entrepreneurship Cell of IGIT Sarang.",
    skills: [
     "Leadership",
     "Entrepreneurship",
    ],
    doc: "",
  },
  {
    id: 1,
    img: "https://indianrailways.gov.in/railwayboard/images/logo22.png",
    role: "Software Developer Intern",
    company: "East Coast Railways",
    date: "June 2023 - July 2023",
    desc: "Built an Complaint management full stack web app for internal uses of East Coast Railway, Bhubaneswar.",
    skills: [
      "ReactJS",
      "NodeJs",
      "Material UI",
      "HTML",
      "CSS",
      "JavaScript",
      "MYSQL",
    ],
  },
  {
    id: 2,
    img: "https://upload.wikimedia.org/wikipedia/commons/thumb/1/17/BSNL_logo_with_slogan.svg/330px-BSNL_logo_with_slogan.svg.png",
    role: "BSNL Intern",
    company: "BSNL",
    date: "2021",
    desc: "Learn about Networking and Fiber optics.",
  //  doc: "",
  },
  {
    id: 3,
    img: "data:image/jpeg;base64,",
    role: "Android Developer",
    company: "Freelance",
    date: "2020 - Present",
    desc: "In my role as an Android developer, I specialize in crafting innovative applications that I then publish on the Play Store, strategically monetizing them to generate revenue streams.",
    skills: [
      "Android",
       "Java",
       "Cloud Firestore",
      "Mobile Application Development",
      "Kotlin",
      "XML",
      "Figma",
    ],
    doc: "https://play.google.com/store/apps/dev?id=4976844272219805591",
 
  },
  
];

export const education = [
  {
    id: 0,
    img: "https://scontent.fbbi4-1.fna.fbcdn.net/v/t39.30808-6/288436638_4984573268323169_2375316028323613935_n.jpg?_nc_cat=104&ccb=1-7&_nc_sid=be3454&_nc_ohc=9fEf1IbxBeYAX9vaPCL&_nc_ht=scontent.fbbi4-1.fna&oh=00_AfBJOMZRZGQ-uQwR5uRwLNH4SO_noZEjAdR85BRPEBqqnw&oe=64DF0830",
    school: "INDIRA GANDHI INSTITUTE OF TECHNOLOGY, Sarang",
    date: "2020 - 2024",
    grade: "9.05 CGPA",
    desc: "I am currently pursuing a Bachelor's degree in Electronics & Telecommunication Engineering at INDIRA GANDHI INSTITUTE OF TECHNOLOGY, Sarang.",
    degree: "Bachelor of Technology - BTech, Electronics & Telecommunication Engineering",
  },
  {
    id: 1,
    img: "https://scontent.fbbi4-1.fna.fbcdn.net/v/t39.30808-6/305277341_591650679327539_5653723109729623322_n.png?_nc_cat=100&ccb=1-7&_nc_sid=be3454&_nc_ohc=xhHUJiBfWMIAX81p2La&_nc_ht=scontent.fbbi4-1.fna&oh=00_AfABMovZLgfOLncC8NeC7ThG9Elu2T3y9OlsIy0K7Wuhbg&oe=64DFCB37",
    school: "Newton College of +2 Science, Sambalpur",
    date: "2017 - 2019",
    grade: "77.17%",
    desc: "I completed my class 12 high school education at Newton College of +2 Science, Sambalpur, where I studied Science with Information Technology.",
    degree: "CHSE(XII), Science",
  },
  {
    id: 2,
    img: "https://scontent.fbbi4-1.fna.fbcdn.net/v/t39.30808-6/278201702_351049093728292_2792713510110519253_n.jpg?_nc_cat=101&ccb=1-7&_nc_sid=be3454&_nc_ohc=qJ1uATnP1oEAX8ohy1s&_nc_oc=AQlcr7RVflooDGjoiLUWzE2IGSNpjQ6ihYq58Mu6IydtkqeBeROAELbMJoU9cdTfy9A&_nc_ht=scontent.fbbi4-1.fna&oh=00_AfAG0KEqoswffhhIvF8p60RtngomaJ4WIbSgiERnapGrVw&oe=64E068A4",
    school: "SSVM, Titilagarh",
    date: "2006 - 2017",
    grade: "88.83%",
    desc: "I completed my class 10 education at Saraswati Sishu Vidya Mandir, Titilagarh.",
    degree: "BSE(X)",
  },
];

export const projects = [
  {
    id: 0,
    title: "Vistaar",
    date: "2022",
    description:
      "The Event Management App for Electronics and Telecommunication Department is a mobile application designed to facilitate the planning, promotion, registration and management of events held at the college. The app will provide an easy and convenient way for organizers to create and manage events, as well as for attendees to stay informed and engaged.",
    image:
      "https://github.com/Biswa1045/Vistaar-igit/assets/92640713/6cf77981-c1dd-4949-a110-a02833ef7631",
    tags: [
     "JAVA",
     "Android Studio",
     "Firebase"

    ],
    category: "android app",
    github: "https://github.com/Biswa1045/Vistaar-igit",
    webapp: "https://play.google.com/store/apps/details?id=com.biswa1045.vistaar",
  },
  {
    id: 1,
    title: "OUAT Preparation App",
    date: "2020",
    description:
      "The OUAT Exam Preparation App is a mobile app developed using Android Studio and Firebase. The app will provide students with questions for practice and mock tests to help them prepare for the OUAT exam. The questions will be presented in PDF format, and the app will also include additional resources such as study materials and video tutorials.",
    image:
      "https://play-lh.googleusercontent.com/y225yUmZK3hCTvDz0eLDnxVig-kRp5nLxKX_v_zDwYJs4ncr2Ka3vjcrAfvz1MJ4IgA=w1052-h592-rw",
    tags: [
     "JAVA",
     "Android Studio",
     "Firebase",
     "Kotlin"

    ],
    category: "android app",
    github: "https://github.com/Biswa1045/OUAT",
    webapp: "https://play.google.com/store/apps/details?id=com.biswa1045.ouat",
  },
  {
    id: 2,
    title: "Quiz Game",
    date: "Dec 2020 - Jan 2021",
    description:
      "The Quiz Game Android App is an exciting and interactive game designed for mobile devices using Android Studio and Java. The app includes a database of questions and answers from various topics, including sports, science, history, and more.The app features a user-friendly interface and a simple navigation system that allows users to select the desired topic and difficulty level. The game includes multiple-choice questions and users have a limited time to answer the questions.The app also includes features such as high score tracking, leaderboard, and social media sharing, allowing users to share their results with friends and challenge them to play the game.The app is designed using Java and Android Studio, and is compatible with a wide range of Android devices. The app is easy to install and requires minimal storage space on the user's device.",
    image:
      "https://play-lh.googleusercontent.com/HM3emwJsC8B5Z_7Sn2pxOvf1Z9XR79XG3fl8mX233HWRqrncCGwTxK3svjg-kOnxLg=w480-h960-rw",
    tags: [
     "JAVA",
     "Android Studio",
     "Firebase",
     

    ],
    category: "android app",
    github: "https://github.com/Biswa1045/QuizGame",
    webapp: "https://play.google.com/store/apps/details?id=com.biswa1045.quizgame",
  },
  {
    id: 3,
    title: "Akash",
    date: "2022",
    description:
      "Shop Stoke management, Bill management app. Streamline bill tracking, budget insights, and timely payment reminders for enhanced financial management.",
    image:
      "https://github.com/Biswa1045/Akash/assets/92640713/b9a898da-84ff-4c91-bb4e-4394a930f2fc",
    tags: [
     "JAVA",
     "Android Studio",
     "Firebase",
     "Kotlin"
     

    ],
    category: "android app",
    github: "https://github.com/Biswa1045/Akash",
    webapp: "https://github.com/Biswa1045/Akash",
  },
  {
    id: 4,
    title: "Gully Cricbuzz",
    date: "2020",
    description:
      "The Cricket Match Score and Player Performance Tracking App is designed to provide updates and detailed information about cricket matches, player scores, and individual player performances. The app aims to enhance the user's experience by offering comprehensive match statistics, player profiles, and interactive features that allow fans to stay engaged with the game.",
    image:
      "https://images.unsplash.com/opengraph/1x1.png?auto=format&fit=crop&w=1200&h=630&q=60&mark-w=64&mark-align=top%2Cleft&mark-pad=50&blend-w=1&mark=https%3A%2F%2Fimages.unsplash.com%2Fopengraph%2Flogo.png&blend=https%3A%2F%2Fimages.unsplash.com%2Fphoto-1633783714421-332b7f929148%3Fcrop%3Dfaces%252Cedges%26cs%3Dtinysrgb%26fit%3Dcrop%26fm%3Djpg%26ixid%3DM3wxMjA3fDB8MXxzZWFyY2h8NXx8bm8lMjBpbWFnZXxlbnwwfHx8fDE2OTE1OTMxOTF8MA%26ixlib%3Drb-4.0.3%26q%3D60%26w%3D1200%26auto%3Dformat%26h%3D630%26mark-w%3D750%26mark-align%3Dmiddle%252Ccenter%26blend-mode%3Dnormal%26blend-alpha%3D10%26mark%3Dhttps%253A%252F%252Fimages.unsplash.com%252Fopengraph%252Fsearch-input.png%253Fauto%253Dformat%2526fit%253Dcrop%2526w%253D750%2526h%253D84%2526q%253D60%2526txt-color%253D000000%2526txt-size%253D40%2526txt-align%253Dmiddle%25252Cleft%2526txt-pad%253D80%2526txt-width%253D660%2526txt-clip%253Dellipsis%2526txt%253Dno%252520image%26blend%3D000000",
    tags: [
     "JAVA",
     "Android Studio",
     "Firebase",
     

    ],
    category: "android app",
    github: "",
    webapp: "",
  },
  {
    id: 5,
    title: "Alumini Network",
    date: "2023",
    description:
      "The alumni network platform is a project aimed at creating a centralized online platform that connects graduates from College. The platform will enable alumni to network, exchange ideas, and access job opportunities while keeping them up-to-date with the latest news from their alma mater.",
    image:
      "https://user-images.githubusercontent.com/92640713/261637147-7ce134e1-9eff-499b-903c-fe7e947063ae.jpeg",
    tags: [
     "JAVA",
     "Android Studio",
     "Firebase",
     "Kotlin",

    ],
    category: "android app",
    github: "https://github.com/Biswa1045/AluminiNetwork",
    webapp: "https://github.com/Biswa1045/AluminiNetwork",
  },
  {
    id: 6,
    title: "Complaint Management Website",
    date: "June 2023 - July 2023",
    description:
      "The East Coast Railways Complaint Management System provides easy complaint registration, login, and tracking. Users can monitor their complaint status. Admins delegate complaints to dealers for resolution. Built with HTML, CSS, JavaScript, Node.js, and MySQL, this system streamlines communication for better customer satisfaction.",
    image:
      railways,
    tags: [
     "JavaScript",
     "html",
     "css",
     "Node.js",
     "My SQL"
    ],
    category: "web app",
    github: "https://github.com/Biswa1045/Railway-Complaint-Management",
    webapp: "https://github.com/Biswa1045/Railway-Complaint-Management",
  },
  
  {
    id: 8,
    title: "360 Camera",
    date: "April 2023",
    description:
      "The Automated 360-Degree Rotating Camera System captures immersive footage by rotating a 360-degree camera around a central subject. This project creates an electronics-controlled setup for enhanced content creation in applications like virtual reality and entertainment.",
    image:
    camera_360,
    tags: [
     "Electonics",
     "Arduino",
     "Welding",
    

    ],
    category: "electronics",
    github: "",
    webapp: "",
  },
  {
    id: 9,
    title: "Ameoba",
    date: "2023",
    description:
      "The water cleaner machine is a device designed to clean the surface of a water body such as a pond, lake or swimming pool. It operates using a suction mechanism that pulls water and debris through the machine, removing dirt, leaves and other pollutants from the surface of the water.The machine consists of a motor, suction pump, and filter, along with a collection bin for holding debris. The device is placed on the surface of the water and turned on, creating a suction that draws water and debris through the machine's filter. The filter captures the debris, while the clean water is returned back to the water body.",
 //   image:"https://media.licdn.com/dms/image/C5622AQHivTmPMrNzcw/feedshare-shrink_1280/0/1677008798113?e=1695254400&v=beta&t=GZtDPNJ3bDeGVWJ7M6Vm3FCe5do67gP3p8pjBJm29Hw",
    image:amoeba,
   
    tags: ["Arduino", "Electronics"],
    category: "electronics",
    github: "https://www.linkedin.com/feed/update/urn:li:activity:7033884715124228096/",
    webapp: "https://www.linkedin.com/feed/update/urn:li:activity:7033884715124228096/",
    member: [
      {
        name: "Biswajit Sahu",
        img: "https://media.licdn.com/dms/image/D5603AQH3peDrI5cNqA/profile-displayphoto-shrink_800_800/0/1685460687516?e=1697673600&v=beta&t=UEPb0LGMCj9SxfR1q0z0YSmkEHOX7AbLaZPvxlYNe7o",
        linkedin: "https://www.linkedin.com/in/biswajit-1045s/",
        github: "https://www.linkedin.com/in/biswajit-1045s",
      },
      {
        name: "Soumya Prakash Mishra",
        img: "https://media.licdn.com/dms/image/D4D03AQEtaBGjXB3TtQ/profile-displayphoto-shrink_400_400/0/1692027014374?e=1697673600&v=beta&t=gQP_YjbtjNor61HCXWziFlxc_3WTP34hZp-t8_GlQxM",
        linkedin: "https://www.linkedin.com/in/biswajit-1045s/",
        github: "https://www.linkedin.com/in/biswajit-1045s",
      },
      {
        name: "Ashish Mishra",
        img: "https://media.licdn.com/dms/image/C4E03AQGRUdJKSYNCvg/profile-displayphoto-shrink_400_400/0/1641617856474?e=1697673600&v=beta&t=V4fqYiP_75mqKpWq6Zbu6EJZBNp-lj3WqW9i8GpXrig",
        linkedin: "https://www.linkedin.com/in/ashish-akm/",
        
      },
      {
        name: "Isha Priyadarshani",
        img: "https://media.licdn.com/dms/image/D5603AQEYJvFtMbfVIA/profile-displayphoto-shrink_100_100/0/1683023896272?e=1697673600&v=beta&t=Jfh6LMRsWOdL8Gx0nDOcnje04HlzydvGFIqHbEHKyhc",
        linkedin: "https://www.linkedin.com/in/isha-priyadarshani-panda-18a1bb233",
        
      },
    ],
  },
  {
    id: 10,
    title: "Live Health Checker",
    date: "Oct 2021 - Jan 2022",
    description:
      "The Health Monitoring System is a project designed to measure and monitor a person's heart rate, body temperature, and oxygen level in real-time. The system uses an Arduino Nano board, GSM module, oximeter, temperature sensor, and heartbeat sensor to collect the data. The data is then transmitted to a cloud-based storage service using ThingSpeak API, and displayed in a mobile application developed using Android Studio.",
 //   image:"https://media.licdn.com/dms/image/C5622AQHivTmPMrNzcw/feedshare-shrink_1280/0/1677008798113?e=1695254400&v=beta&t=GZtDPNJ3bDeGVWJ7M6Vm3FCe5do67gP3p8pjBJm29Hw",
    image:"https://images.unsplash.com/opengraph/1x1.png?auto=format&fit=crop&w=1200&h=630&q=60&mark-w=64&mark-align=top%2Cleft&mark-pad=50&blend-w=1&mark=https%3A%2F%2Fimages.unsplash.com%2Fopengraph%2Flogo.png&blend=https%3A%2F%2Fimages.unsplash.com%2Fphoto-1633783714421-332b7f929148%3Fcrop%3Dfaces%252Cedges%26cs%3Dtinysrgb%26fit%3Dcrop%26fm%3Djpg%26ixid%3DM3wxMjA3fDB8MXxzZWFyY2h8NXx8bm8lMjBpbWFnZXxlbnwwfHx8fDE2OTE1OTMxOTF8MA%26ixlib%3Drb-4.0.3%26q%3D60%26w%3D1200%26auto%3Dformat%26h%3D630%26mark-w%3D750%26mark-align%3Dmiddle%252Ccenter%26blend-mode%3Dnormal%26blend-alpha%3D10%26mark%3Dhttps%253A%252F%252Fimages.unsplash.com%252Fopengraph%252Fsearch-input.png%253Fauto%253Dformat%2526fit%253Dcrop%2526w%253D750%2526h%253D84%2526q%253D60%2526txt-color%253D000000%2526txt-size%253D40%2526txt-align%253Dmiddle%25252Cleft%2526txt-pad%253D80%2526txt-width%253D660%2526txt-clip%253Dellipsis%2526txt%253Dno%252520image%26blend%3D000000",
   
    tags: ["Arduino", "Electronics", "Android", "ThingSpeak", "Proteus"],
    category: "electronics",
    github: "",
    webapp: "",
    
  },
  

 
 
  
 
];

export const TimeLineData = [
 
  { year: 2020, text: "Starting from Android" },
  { year: 2021, text: "Play with Some Low Voltage Stuffs" },
  { year: 202, text: "Entrepreneurship" },
  { year: 2023, text: "Backside of Internet" },
];
